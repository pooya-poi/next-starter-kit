...
function SidebarProvider({
  ...
  return (
    <SidebarContext.Provider value={contextValue}>
      ...
      <div {...handlers} className="fixed z-30 h-[100vh] w-5.5 md:hidden"></div>
...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      size="icon"
      className={cn(
        'hover:bg-background size-8 cursor-pointer rounded-full',
        className
      )}
      onClick={event => {
        onClick?.(event);
    ...
    >
      <ChevronLeft
        className={`text-foreground/50 absolute top-1 left-1 p-0.5 ${state === 'collapsed' ? 'rotate-180' : ''}`}
      />
...
function SidebarTriggerMobile({
  ...
  return (
    <Button
      ...
      size="icon"
      className={cn(
        'hover:bg-background relative block size-8 cursor-pointer rounded-xl md:hidden',
        className
      )}
      onClick={event => {
        onClick?.(event);
...
function SidebarMenu({ className, ...props }: React.ComponentProps<'ul'>) {
  return (
    <ul
      ...
      data-sidebar="menu"
      className={cn('flex w-full min-w-0 flex-col gap-1 pr-2', className)}
      {...props}
    />
  );
}
